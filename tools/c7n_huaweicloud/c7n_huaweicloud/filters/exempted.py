        ...
        if isinstance(value, list):
            for item in value:
                if not isinstance(item, (str, int)):
                    log.error(f"{field}-{instance(value)} type error in resource")
                    raise PolicyValidationError(f"{field}-{instance(value)} type error in resource")
            return value
        log.error(f"{field}-{instance(value)} type error in resource")
        raise PolicyValidationError(f"{field}-{instance(value)} type error in resource")
    else:
        log.error(f"{field} is required in resource")
        raise PolicyValidationError(f"{field} is required in filter")
...
def get_tags_from_resource(resource):
    try:
        ...
        log.error("Parse Tags in resource %s failed", resource["id"])
        return {}
...
class ExemptedFilter(Filter):
    ...
    def __call__(self, i):
        ...
        try:
            ...
            self.log.error(f"get_values_from_resource error, auto filtered, "
                           f"resource {i['id']}, filed {field}")
            return False
    ...
    def get_exempted_values_from_obs(self, obs_url, group_key):
        try:
            ...
            self.log.error(e.status_code, e.request_id, e.error_code, e.error_msg)
            raise
        except Exception:
...
class RestrictedFilter(Filter):
    ...
    def __call__(self, i):
        ...
        return len(resource_values & restricted_values) == len(restricted_values)
