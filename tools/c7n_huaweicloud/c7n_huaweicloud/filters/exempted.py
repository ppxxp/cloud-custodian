    ...
    def get_exempted_values_from_obs(self, obs_url, group_key):
        try:
            ...
            if 300 > resp.status >= 200:
                ...
                log.error(f"get obs object failed: "
                               f"{resp.errorCode}, {resp.errorMessage}, {resp.requestId}")
                raise HTTPError(resp.status, resp.body)
        except exceptions.ClientRequestException as e:
            log.error("get obs object failed, ", e.status_code, e.request_id,
                           e.error_code, e.error_msg)
            raise
        except Exception:
            log.error(f"get exempted values from obs occur exception, "
                      f"obs_url:[{obs_url}], group_key:[{group_key}]")
            raise
