    ...
    def process_resource(self, resource, old_key, new_key, value):
        try:
            ...
            if len(delete_response.failed_resources) > 0:
                self.log.warning(f"[actions]-rename-tag The resource:{self.manager.ctx.policy.resource_type} "
                ...
                self.log.debug(f"[actions]-rename-tag The resource:{self.manager.ctx.policy.resource_type} "
                               f"with id:[{resource['id']} "
...
class NormalizeResourceTagAction(HuaweiCloudBaseAction):
    ...
    def process_resource(self, resource):
        try:
            ...
            delete_response = self.tms_client.delete_resource_tag(request=request)
            if len(delete_response.failed_resources) > 0:
                self.log.warning(f"[actions]-normalize-tag The resource:{self.manager.ctx.policy.resource_type} "
                                 f"with id:[{resource['id']} "
                                 f"delete tag:{old_tags} is failed. "
                                 f"cause: {delete_response.failed_resources[0].error_msg}")
                self.handle_exception(failed_resources=[resource], resources=self.resources)
                ...
                self.log.debug(f"[actions]-normalize-tag The resource:{self.manager.ctx.policy.resource_type} "
                               f"with id:[{resource['id']} "
                               f"delete tag:{old_tags} is success. ")
...
class TrimResourceTagAction(HuaweiCloudBaseAction):
    ...
    def process_resource(self, resource, space, preserve):
        try:
            ...
            delete_response = self.tms_client.delete_resource_tag(request=request)
            if len(delete_response.failed_resources) > 0:
                self.log.warning(f"[actions]-tag-trim The resource:{self.manager.ctx.policy.resource_type} "
                                 f"with id:[{resource['id']} "
                                 f"delete tag:{old_tags} is failed. "
                                 f"cause: {delete_response.failed_resources[0].error_msg}")
                self.handle_exception(failed_resources=[resource], resources=self.resources)
                ...
                self.log.debug(f"[actions]-tag-trim The resource:{self.manager.ctx.policy.resource_type} "
                               f"with id:[{resource['id']} "
                               f"delete tag:{old_tags} is success. ")
        except exceptions.ClientRequestException as ex:
            self.log.warning(
